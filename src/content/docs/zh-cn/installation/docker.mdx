...
Docker Compose 是部署 EasyShell 的推荐方式。所有服务（Server、Web、MySQL、Redis）均已完整容器化，预构建镜像通过 GitHub Actions 自动发布到 GHCR 和 Docker Hub。
...
无需本地构建 —— 预构建镜像会自动从 [GHCR](https://github.com/orgs/easyshell-ai/packages) 拉取。
...
| 服务 | 镜像 | 端口 | 说明 |
|------|------|------|------|
| **easyshell-server** | `ghcr.io/.../easyshell-server:latest` | `18080` | Java Spring Boot 后端 + 内置 Agent 二进制 |
| **easyshell-web** | `ghcr.io/.../easyshell-web:latest` | `18880` | React 前端（Nginx 托管） |
...
# Docker 镜像源（默认 GHCR，取消注释切换到 Docker Hub）
